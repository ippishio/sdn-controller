...
s1 = network.addSwitch("s1", cls=OVSSwitch, dpid="1")
s2 = network.addSwitch("s2", cls=OVSSwitch, dpid="2")
br_ex = network.addSwitch("br-ex", cls=OVSBridge, ip="10.0.0.254/24", dpid=str(0x1234))
...
network.start()
# отключаем контроллер от бриджа
br_ex.cmd("ovs-vsctl set bridge br-ex datapath_type=system")
br_ex.cmd("ovs-vsctl set-fail-mode br-ex standalone")
br_ex.cmd("ovs-vsctl set bridge br-ex protocols=[]")
